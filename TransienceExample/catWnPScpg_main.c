...
int main (void) { 
  ...
  for (b=0; b<1; b++)  //Use this for loop if you are sweeping a parameter, otherwise keep it as 
	...
	{                    //keep it at (c=0; c<1; c++)
	  ...
	  gModWE = 0.0;
	  rr1=integrate(run1, run2, run3, run4, 1, tf1, tint, nvar, yy);  //For this example, we are integrating until we reach a specific phase of walking cycle.
	  gModWE = gMod0;                                             //Pulse is turned on.
	  rr2=integrate(run1, run2, run3, run4, 2, tf2, tint, nvar, yy);  //Pulse is applied. integrate() updates the values contained in yy.
	  gModWE = 0.0;                                               //Pulse is turned off.
	  rr3=integrate(run1, run2, run3, run4, 3, tf3, tint, nvar, yy);  //Pulse is turned off again. Runs for a while to reach passed the end 
                                                                            //of transience. Due to file size limits and memory limits. Only about 30 sec
                                                                            //can be run at once, but that might depend on what computer you are using.
